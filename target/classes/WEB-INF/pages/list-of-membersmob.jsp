...
<body>
			...
			<div class="collapse navbar-collapse"
				...
				<ul class="nav navbar-nav">
					...
					<li><a href="https://apps.cep.ch/cep/membres.php" target=blank> Membres
							Mobile (M)<span style="font-size: 16px;"
							class="pull-right hidden-xs showopacity glyphicon glyphicon-phone"></span>
					</a></li>
	...
	<p>Session utilisateur <b>${pageContext.request.userPrincipal.name}</b></p>
		<h4>Liste des Membres Mobile</h4>
		<br />
		<c:if test="${not empty message}">
			<div class="alert alert-success">${message}</div>
		</c:if>
		<!--Add Company + Export to Excel (XSLX)-->
		<div>
			<p class="h6, col-md-9">
				<a href="<c:url value='/'/>" class="btn btn-info custom-width">Home</a>
				<a href="${pageContext.request.contextPath}/membermob/add.html"><strong>+
						Membre Mobile</strong></a>
			</p>
			<p align="right">
				<input type="button"
					onclick="tableToExcel('membersmob', 'membersmob')" value="Excel"
					class="btn btn-warning" disabled="disabled"><br />
			</p>
		</div>
		<!--MembersMob List-->
		<table id="data" class="table table-striped table-hover">
			...
			<tbody>
				<c:forEach var="memberMob" items="${membersMob}">
					<c:if test="${memberMob.memberMobIsActive==true}">
						<tr>
							<!--tr class="info"-->
							<td><p class="h6">${memberMob.memberMobFirstName}</p></td>
							<td><p class="h6">${memberMob.memberMobLastName}</p></td>
							<td><p class="h6">${memberMob.memberMobCompany}</p></td>
							<td><p class="h6">${memberMob.memberMobEmail}</p></td>
							<td><p class="h6">${memberMob.memberMobPassword}</p></td>
							<td><p class="h6">${memberMob.memberMobIdentifier}</p></td>
							<td><p class="h6">
									<fmt:formatDate
										value="${memberMob.dateMemberMobIdentification}"
										var="formattedIdentificationDate" type="date"
										pattern="dd.MM.yyyy - HH:mm" />${formattedIdentificationDate}</p></td>
							<td><p class="h6" align="center">
								<table>
									<tr>
										<td><p class="h6, col-md-3, info">
												<a
													href="${pageContext.request.contextPath}/membermob/view/${memberMob.id_memberMob}.html"
													data-toggle="tooltip" title="Détail"
													class="glyphicon glyphicon-eye-open"></a>
											</p></td>
										<td><p class="h6, col-md-3, info">
												<a
													href="${pageContext.request.contextPath}/membermob/edit/${memberMob.id_memberMob}.html"
													title="Edition" class="glyphicon glyphicon-edit"></a>
											</p></td>
										<td><p class="h6, col-md-3, info">
												<a
													href="${pageContext.request.contextPath}/membermob/delete/${memberMob.id_memberMob}.html"
													title="Suppression" class="glyphicon glyphicon-trash" onclick="return confirm('Etes-vous sûr de vouloir désactiver/effacer ce membre mobile?')"></a>
											</p></td>
									</tr>
								</table>
						</tr>
