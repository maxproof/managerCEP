...
table td[value="${Club-Entreprises}"] {
    color: red;
}
</style>
<style>
	...
	<p>Session utilisateur <b>${pageContext.request.userPrincipal.name}</b></p>
		...
		<div>
			<p class="h6, col-md-9">
				...
				<a href="${pageContext.request.contextPath}/company/add.html"><strong>+
						Entreprise</strong></a>
			</p>
			<p align="right">
			<a href="<c:url value='/export-companies-xls'/>" class="btn btn-warning">Excel</a>
			</p>
		</div>
		<!--Companies List-->
		<table id="data" class="table table-striped table-hover">
			...
			<tbody>
				<c:forEach var="company" items="${companies}">
					<c:if test="${company.companyIsActive==true}">
						<tr>
							<c:forEach items="${companiesListCategoriesList}" var="category">
								<c:if test="${company.companyCategory==category.key}">
									<td class="category4company"><p class="h6">${category.value}</p></td>
								</c:if>
							</c:forEach>
							<td class="companycat"><p class="h6">${company.companyName}</p></td>					
							<td><p class="h6">${company.companyStreet}</p></td>
							<td><p class="h6">${company.companyPostCode}</p></td>
							<td><p class="h6">${company.companyCity}</p></td>
							<td><p class="h6">${company.companyPhoneNumber}</p></td>
							<td><p class="h6">${company.companyEmail}</p></td>
							<!--td><p class="h6">${company.companyComments}</p></td-->
							<td><p class="h6">
									<fmt:formatDate
										value="${company.companyInsertionTimestamp}"
								...
								<table>
									<tr>
										<td><p class="h6, col-md-3, info">
												<a
													href="${pageContext.request.contextPath}/company/view/${company.id_company}.html"
													title="Détail" class="glyphicon glyphicon-eye-open"></a>
											</p></td>
										<td><p class="h6, col-md-3, info">
												<a
													href="${pageContext.request.contextPath}/company/edit/${company.id_company}.html"
													title="Edition" class="glyphicon glyphicon-edit"></a>
											</p></td>
										<td><p class="h6, col-md-3, info">
												<a
													href="${pageContext.request.contextPath}/company/delete/${company.id_company}.html"
													title="Suppression" class="glyphicon glyphicon-trash" onclick="return confirm('Etes-vous sûr de vouloir désactiver/effacer cette entreprise?')"></a>
											</p></td>
									</tr>
								</table>
						</tr>
